...
class Solution:
    ...
    def dfs(cls, n: int) -> List[List[int]]:
        def helper(queue: List[int], xy_diff: List[int], xy_sum: List[int], check_n: int,
                   tmp_res: List[List[int]]) -> None:
            # terminator
            ...
            for col in range(n):
                if col not in queue and row - col not in xy_diff and row + col not in xy_sum:
                    # drill down
                    helper(queue + [col], xy_diff + [row - col], xy_sum + [row + col], check_n, tmp_res)
